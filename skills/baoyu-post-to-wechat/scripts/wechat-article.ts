  ...
  try {
    ...
    if (!chrome) {
      ...
      if (wechatTab) {
        ...
        if (!currentUrl.includes('/cgi-bin/home') || currentUrl.includes('action=edit')) {
    ...
    const formReady = await waitForElement(session, '#title', 10_000);
    if (!formReady) {
      console.warn('[wechat] Title input not found, retrying...');
      await sleep(3000);
    }

    if (effectiveTitle) {
      console.log('[wechat] Filling title...');
      // Clear first, then type with focus
      await evaluate(session, `
        (function() {
          ...
          titleEl.focus();
          titleEl.select();
          titleEl.value = ${JSON.stringify(effectiveTitle)};
    ...
    if (options.images && options.images.length > 0) {
      const coverImage = options.images[0];
      console.log(`[wechat] Uploading cover image: ${coverImage}`);
      
      // Find file input and upload via CDP
      const uploadSuccess = await evaluate<boolean>(session, `
        (function() {
          // Try to find hidden file input
          let fileInput = document.querySelector('input[type="file"]');
          
          if (!fileInput) {
            // Try cover upload area
            const coverArea = document.querySelector('#js_cover_area') || 
                             document.querySelector('.cover__wrapper') ||
                             document.querySelector('.cover__hd');
            if (coverArea) {
              coverArea.click();
              // File input might be created dynamically
              setTimeout(() => {
      ...
      if (uploadSuccess) {
        ...
        // Try to set file via CDP (this works for some sites)
        try {
          const result = await session.cdp.send('DOM.querySelector', {
          ...
          if (result.nodeId) {
            ...
            console.log('[wechat] Cover image uploaded via CDP');
          }
        } catch (err) {
          console.warn('[wechat] CDP file upload failed, dialog may require manual selection');
        }
        
        await sleep(3000);
      } else {
        console.warn('[wechat] Could not find cover upload area');
      }
    }
    ...
    // Click save button with multiple attempts
    const saveClicked = await evaluate<boolean>(session, `
      (function() {
        ...
        // Fallback: find button with "保存" or "save" text
        const buttons = document.querySelectorAll('button, a[role="button"]');
        for (const btn of buttons) {
          const text = btn.textContent?.toLowerCase() || '';
          if (text.includes('保存') || text.includes('save') || text.includes('存稿')) {
            btn.click();
            return true;
          }
        }
    ...
    if (!saveClicked) {
      console.warn('[wechat] Could not find save button, trying keyboard shortcut...');
      // Try keyboard shortcut Ctrl+S / Cmd+S
      const modifiers = process.platform === 'darwin' ? 4 : 2;
      await session.cdp.send('Input.dispatchKeyEvent', { type: 'keyDown', key: 's', code: 'KeyS', modifiers, windowsVirtualKeyCode: 83 }, { sessionId: session.sessionId });
      await sleep(50);
      await session.cdp.send('Input.dispatchKeyEvent', { type: 'keyUp', key: 's', code: 'KeyS', modifiers, windowsVirtualKeyCode: 83 }, { sessionId: session.sessionId });
    }
    
    // Wait longer for save confirmation
    ...
    const saveSuccess = await evaluate<boolean>(session, `
      (function() {
        ...
        const successMsg = document.querySelector('.weui-desktop-toast__text');
        if (successMsg && successMsg.textContent?.includes('成功')) return true;
        
        // Check if we're still on edit page (no navigation happened)
        const url = window.location.href;
        if (url.includes('/cgi-bin/appmsg')) return true; // Still on edit page = save in progress
    ...
    if (saveSuccess) {
      ...
      console.log('[wechat] Save confirmation not detected, but save may still be in progress.');
      await sleep(3000);
    }
